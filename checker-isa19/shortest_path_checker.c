...
int just(Graph *g, EInt *dist, int *c, unsigned int s, unsigned int *enu, int *pred) {
...
int no_neg_dist(Graph *g, EInt *dist) {
    for(unsigned int v = 0; v < vertex_cnt(g); v++) {
...
int check_basic_just_sp(Graph *g, EInt *dist, int *c, unsigned int s, unsigned int *enu, int *pred) {
...
int check_sp(Graph *g, EInt *dist, int *c, unsigned int s, unsigned int *enu, int *pred) {
    if(!check_basic_just_sp(g, dist, c, s, enu, pred)) return 0;
    
if(dist[s].isInf != 0) return 0;
    if(dist[s].val != 0) return 0;
    if(!no_neg_dist(g, dist)) return 0;
