...
print_theorems
find_theorems name:"prod.simp"
interpretation counterexample_sp_inf: 
  sp_inf
      counterexample_graph (*G*)
      "\<lambda>x. 1" (*c*)
      "False" (*s*)
  ...
  apply (clarsimp simp: counterexample_sp_plus.sp_plus_axioms)
  apply (rule conjI) 
   unfolding pre_digraph.cycle_def pre_digraph.awalk_def counterexample_graph_def
   apply clarsimp
   apply (smt counterexample_graph_def insertI1 insert_commute list.simps(15) 
              pre_digraph.simps(3) pre_digraph.simps(4) prod.sel(1) prod.sel(2) 
              singletonD subset_eq counterexample.cas.elims)
  using counterexample.U_finite_def counterexample_graph_def 
  apply simp
  done
   
interpretation counterexample_sp_trian: 
 sp_trian
      counterexample_graph (*G*)
      "\<lambda>x. 1" (*c*)
      "False" (*s*)
  ...
  using counterexample_sp_inf.sp_inf_axioms 
  by simp
...
interpretation counterexample_sp_trian_just: 
 sp_trian_just 
  ...
  using counterexample_sp_trian.sp_trian_axioms
  by (simp add: counterexample.U_finite_def)
