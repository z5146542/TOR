...
(is 
  ...
  using is_dist_def is_graph_def
  Word.sint_0 int_unat not_le is_inf_d_heap s_C_pte t_C_pte two_comp_to_edge_arrlist_heap
 wellformed_iGraph 
 try0
  apply (subst is_inf_d_heap, simp add: is_dist_def) 
...
definition pwalk_int_neg_cyc_nth_inv :: 
  ...
  where
  "pwalk_int_neg_cyc_nth_inv G sc d cse c p n v u is_neg_cycle j k \<equiv>
    \<forall>j'<j. \<forall>i<k. is_inf_d d v < 0 \<and>
...
lemma pwalk_int_neg_cyc_nth_inv_le:
  assumes leq: "i \<le> m" 
  assumes inv: "pwalk_int_neg_cyc_nth_inv G sc d cse c p n v u is_neg_cycle j m"
  shows "pwalk_int_neg_cyc_nth_inv G sc d cse c p n v u is_neg_cycle j i"
  using assms 
  oops
...
(is 
  ...
  apply wpsimp
  apply (subst whileLoopE_add_inv [where 
            M="\<lambda>(v, s). ?verts - v" and
            I="\<lambda>v s. ?pre s \<and> v \<le> ?verts \<and> ?inv v"])
  defer
  apply wp+
  apply (rule conjI)
  apply (rule conjI)
  apply assumption
  apply (clarsimp simp: is_graph_def )
  apply (clarsimp simp:  are_cycles''_def)
  apply wp
  apply (clarsimp)
  apply (subst whileLoop_add_inv [where 
               M = "\<lambda> ((is_neg_cycle, j, u), s). iN u - j" and 
               I = "\<lambda>(is_neg_cycle, j, u) s. 
                    ?pre s \<and> v \<le> ?verts \<and> ?inv v \<and> 
                    is_inf_d iD v < 0 \<and> j \<le> iN v \<and> 
                    pwalk_int_neg_cyc_inv
                      iG sc iD iY iC iP iN v u is_neg_cycle j" 
                      for v])
  apply wp
    ...
    apply (subst whileLoop_add_inv [where 
               ...
               I = "\<lambda>(i, b) s. 
                  ?pre s \<and> v \<le> ?verts \<and> ?inv v \<and> 
                    is_inf_d iD v < 0 \<and> j \<le> iN v \<and>
                    unat i \<le> length iY \<and>
                    pwalk_int_neg_cyc_nth_inv
                      iG sc iD iY iC iP iN v u is_neg_cycle j 
                      (unat i)" for v is_neg_cycle j u] )
  ...
  unfolding  pwalk_int_neg_cyc_impl_inv[symmetric] sledgehammer
  unfolding pwalk_int_neg_cyc_nth_inv_def
