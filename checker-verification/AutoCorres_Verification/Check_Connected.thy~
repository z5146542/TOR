...
lemma (in connected_components_locale) vwalk_length_pwalk:
  ...
  proof -
    have "pwalk v = pwalk (tail G (the (parent_edge v))) \<oplus> [tail G (the (parent_edge v)), v]"
    by (meson assms(1) assms(2) pwalk.simps)
  then show ?thesis
    by (metis (no_types) One_nat_def add_cancel_left_left add_diff_cancel_right' 
          length_append length_tl list.size(3) list.size(4) pwalk_ne tl_append2 vwalk_join_def 
          vwalk_length_simp)
  qed
    ...
    case False
        ...
        by (metis pp ew pp'(1) jp vwalk_joinI_vwalk)
      {
        ...
        have "vwalk_length (pwalk x) \<le> vwalk_length ?p'"
          using pwalk_split[OF xp]
            by (smt length_append vwalk_length_simp less_or_eq_imp_le
                Nat.add_diff_assoc2 add_lessD1 less_not_refl  
                linorder_neqE_nat neq0_conv zero_less_diff)
