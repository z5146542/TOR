(* uses Isabelle2017 and autocorres version 1.4 *)
      ...
      heap_ptr_coerce word_zero_le two_comp_to_edge_arrlist_heap t_C_pte)
       apply (rule conjI, rule impI, rule conjI, rule impI, rule conjI)
        ...
        apply (rule conjI)
         ...
         apply (clarsimp simp: if_bool_eq_conj)+
         apply (rule arrlist_nth, (simp add: uint_nat unat_mono )+) 
         apply (metis not_le s_C_pte two_comp_to_edge_arrlist_heap word_le_nat_alt)
        apply (rule conjI)
         apply (unfold is_graph_def just_inv_def is_dist_def is_cost_def is_numm_def is_pedge_def wf_digraph_def)[1]
         apply (clarsimp simp: if_bool_eq_conj)+
         apply (rule arrlist_nth, (simp add: uint_nat unat_mono is_graph_def)+) 
...
lemma trian_inv_eq_math:
  ...
  using less_ereal.simps(2) 
   apply fastforce
  apply (subgoal_tac "\<not> True \<or> snd (d (snd (snd (snd G) i))) = 0")
   apply (subgoal_tac "\<not> True \<or> unat (fst (d (snd (snd (snd G) i)))) \<le> 
                         unat (fst (d (fst (snd (snd G) i)))) + unat (c i)")
    apply (simp add: long_ucast word_add_cast_up_no_overflow word_le_nat_alt)
   apply fastforce
  apply fastforce
  done
...
proof -
  ...
  apply clarsimp by (rule iffI, auto, fastforce simp: abs_IDist_def)
qed
