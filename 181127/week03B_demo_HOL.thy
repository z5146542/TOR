...
text {* Technical Setup, introducing a new logic *}
...
text {* Additional concrete syntax *}
...
text {* Axioms and basic definitions *}
...
text {* Deriving the standard proof rules in HOL *}
...
-- {* we want to show @{text "\<lbrakk> P \<longrightarrow> Q; P; Q \<Longrightarrow> R \<rbrakk> \<Longrightarrow> R"} *}
lemma impE:
